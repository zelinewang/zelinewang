...
| 2026-05-01 | ~~**CSS `@keyframes` inside `<style>` does NOT animate when SVG is loaded via `<img>` on GitHub.**~~ **CORRECTED 2026-05-02 (see Slice 6 lesson below):** `@keyframes` inside an INLINE `<style>` element of the SVG DOES animate — the Platane snake proves it. The original blanket rule was wrong. Where the rule does apply: external CSS files, or `<script>` blocks (those ARE blocked). For pure SMIL alternatives, `<animate>` / `<animateTransform>` also work. | Slice C lesson, corrected after Slice 6 empirical verification |
...
- **2026-05-02 (Slice 6)** — **Slice 6 deliverables shipped** (cron stats actually run + snake embedded in mega-SVG + SMIL ambient pulses + section-aligned link strips). Concrete artifacts: (a) `.github/workflows/refresh-stats.yml` daily 04:00 UTC; (b) `.github/scripts/render-profile.mjs` (181 lines, 22 tokens computed including label X positions); (c) 3 `.github/templates/*.svg.template` files; (d) `previews/*/assets/01-profile.svg` regenerated nightly. **Pending sync into `vibe-readme/SKILL.md`**: 9 new lessons in the "v3 / Slice 6 build" table below — particularly (1) the `@keyframes` correction to the earlier blanket rule, (2) the image-map → section-aligned-link-strip choice, (3) "alive ≠ continuous everywhere fast" ambient SMIL discipline, (4) the render-script-vs-template token-ownership rule, (5) `qlmanage` as a free local SVG previewer.
...
### New confirmed lessons (from v3 / Slice 6 build — alive + cron-actual + snake)

| Date | Lesson | Source |
|---|---|---|
| 2026-05-02 | **Cron stats refresh actually shipped (Phase 2 done).** `refresh-stats.yml` runs daily at 04:00 UTC + on-demand + after Generate Snake completes. Calls `node .github/scripts/render-profile.mjs` which (a) fetches `users/zelinewang` + `users/zelinewang/events?per_page=100` via `gh api`, (b) computes 22 derived tokens including bar widths AND label X positions, (c) reads 3 `.svg.template` files, (d) substitutes `{{TOKEN}}` placeholders, (e) writes 3 rendered mega-SVGs. Commits only if any SVG byte-changed (idempotent). Stats now drift weekly without manual re-render. | Slice 6 cron infrastructure |
| 2026-05-02 | **`execFileSync` from `node:child_process` is the safe shell-free way to call gh from Node.** Pre-existing security hook flags string-form spawning as a shell-injection vector even when args are constants. The argv-array form skips the shell entirely — same reliability, no hook noise, no theoretical injection surface. Hardcoded path arg makes it bulletproof. | render-profile.mjs after security hook flagged the string-form call |
| 2026-05-02 | **Render-script is the right home for visual-derived tokens, not the SVG template.** Bar widths come from data (counts vs maxActivity); label X positions come from bar widths + start-x + padding. If the template hard-codes label X, it goes stale the moment the data changes. Compute label X in the render script (`labelX(n) = String(180 + barW(n) + 10)`); template just references `{{PUSH_BAR_LABEL_X}}`. The template stays purely about layout/animation; data math stays in JS. | console.svg.template + render-profile.mjs label-X token bug, fixed in same slice |
| 2026-05-02 | **CSS `@keyframes` inside an inline `<style>` element of an `<img>`-loaded SVG DOES animate on GitHub.** Earlier LEARNINGS line 19 said it doesn't — that is wrong as a blanket rule. Modern browsers honor inline `<style>` (including `@keyframes`) inside SVG, even when the SVG is loaded as `<img>`. The Platane snake (used in `zelinewang/zelinewang` real profile) is 100% CSS keyframes and animates correctly. Where the earlier rule applies: external CSS files referenced from SVG, or `<script>` blocks — those ARE blocked. | Empirical: Platane snake @keyframes in 69KB inline `<style>` tested live in production |
| 2026-05-02 | **Snake URL fetch needs a graceful fallback.** Snake lives at `raw.githubusercontent.com/<user>/<user>/output/github-snake.svg` — that branch is created by another workflow (Generate Snake / `Platane/snk@v3`). Until that workflow runs at least once, the URL 404s. Render script must catch and return placeholder text inside an empty `<g>` zone, NEVER throw — otherwise daily refresh loop dies. Same for HTTP errors / network blips. | render-profile.mjs `fetchSnake()` try/catch + placeholder text |
| 2026-05-02 | **Snake re-tinting via CSS overrides isn't worth it.** First instinct was to re-color the snake greens to match each direction's palette (phosphor for console, starlight for constellation, ink for field-notes). Tried CSS overrides: too brittle (Platane uses inline styles + keyframe-driven fills + `--c0..c4` CSS vars). Just embed the snake unmodified — its native greens read as "contribution heatmap" universally regardless of surrounding aesthetic, and it carries enough visual identity to feel like a deliberate inclusion rather than a mismatched widget. | snakeForDirection() in render-profile.mjs returns snake content as-is after one tinting attempt |
| 2026-05-02 | **HTML `<map>` + `<area>` image-maps DO NOT work for responsive READMEs.** `<area coords="x1,y1,x2,y2">` are pixel-absolute against the source image's intrinsic dimensions. GitHub re-renders the `<img>` at the container width — the coords don't scale, so click targets drift on desktop and break entirely on mobile. Skip image-maps. The honest fallback is a section-aligned link strip below the image (e.g. `§ 02 projects:` ... linking out to repos), one strip row per major SVG section. | Slice 6 attempted image-map design, abandoned for section-aligned link strip |
| 2026-05-02 | **"Alive" comes from a few subtle SMIL ambient pulses, not from a busy animation everywhere.** Console: breathing live dot in title bar (2.4s opacity 1↔0.35), bar-chart draw-in (1.4s width 0→target, 0.15s stagger), CPU value pulse on "running" agents only, cursor blink at the sidekick prompt. Constellation: 5 star opacity pulses (3.4-4.4s desynced cycles), transmit ring r-pulse (3.6s). Field-notes: stamp `stroke-opacity` breath (4.4s), `· · ·` opacity flicker (2.6s). The whole composition reads as alive without becoming a screensaver. **Ambient ≠ continuous, everywhere, fast.** | All 3 v3 templates after user said "make SVG alive/breathe, not just static" |
| 2026-05-02 | **Native macOS `qlmanage` is a free SVG-to-PNG previewer.** `qlmanage -t -s 800 -o /tmp/out file.svg` produces a thumbnail at 800px width using QuickLook's static SVG renderer. No animations execute (snapshot only) but layout, colors, fonts, stat token substitution are all visible — enough to catch render bugs without spinning up a browser. Different output filename collisions: each call produces `<input>.png`, so render to a fresh dir per file. | Slice 6 visual verification when `rsvg-convert` not installed |
